...
def User_Home(request):
    ...
    try:
        ...
        latest_prediction = Search_Data.objects.filter(
            patient=patient
        ).order_by('-id').first()
    except:
        pass
...
def Doctor_Home(request):
    ...
    # ALL prediction/search records
    records = Search_Data.objects.all()

    # Unique patients
    total_patients = records.values('patient').distinct().count()

    healthy_patients = records.filter(result="0")\
                              .values('patient').distinct().count()

    unhealthy_patients = records.filter(result="1")\
                                .values('patient').distinct().count()
    ...
    daily_patients = records.filter(created__date=today)\
                             .values('patient').distinct().count()

    hourly_patients = records.filter(created__gte=last_hour)\
                              .values('patient').distinct().count()

    # Risk distribution
    low_risk = healthy_patients
    moderate_risk = 0
    high_risk = unhealthy_patients
...
def predict_desease(request, pred, accuracy):
    ...
    d = {'pred': pred, 'accuracy':accuracy, 'doctor':doctors}
...
def view_search_pat(request):
    ...
    try:
        ...
        data = Search_Data.objects.filter(patient__address__icontains=doc.address,pred__get=0).order_by('-id')
    except:
        try:
